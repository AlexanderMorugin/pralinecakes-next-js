// упаковка
export enum Pack {
  CAKE = 'Цветная картонная коробка с окошком',
  PASTRY = 'Картонная коробка',
}

// подзаголовок для всех страниц
export const PAGE_SUBTITLE =
  'вкусные кондитерские изделия, десерты, торты, пирожные';

// хлебная крошка
export const PAGE_MAIN = 'Главная';

// заголовки и мета теги страниц
export const PAGE_MAIN_TITLE = 'Кондитерская Пралине';
export const META_MAIN_TITLE =
  'Производитель и поставщик вкусных кондитерских изделий, десертов, тортов и пирожных.';
export const META_MAIN_DESCRIPTION =
  'Замороженные десерты для кафе, баров, ресторанов, кофеен, пекарен, магазинов и театров. Собственное производство, оптовые цены, бесплатная доставка.';

export const PAGE_ABOUT_TITLE = 'О нас';
export const META_ABOUT_TITLE =
  'Мы стабильная команда профессиональных кондитеров обладающих большим опытом.';
export const META_ABOUT_DESCRIPTION =
  'Кондитерская Пралине - это команда опытных профессиональных кондитеров. Наши десерты представлены во многих московских кафе.';

export const PAGE_CONTACTS_TITLE = 'Контакты';
export const META_CONTACTS_TITLE =
  'Контакты кондитерской Пралине. Москва. Телефон +7 977 138-82-64.';
export const META_CONTACTS_DESCRIPTION =
  'Москва, 1-я Магистральная улица, д.18, стр.1. Телефон +7 977 138-82-64. Ежедневно с 10:00 до 19:00.';

export const PAGE_COOPERATION_TITLE = 'Сотрудничество';
export const META_COOPERATION_TITLE =
  'Сотрудничество с кондитерской Пралине. Высокий уровень наших пирожных и тортов увеличит прибыль вашей компании.';
export const META_COOPERATION_DESCRIPTION =
  'Приглашаем к сотрудничеству кафе, театры, детские развлекательные зоны, комплексы, кейтеринговые службы, кофейни, рестораны, бары, пекарни. Становитесь клиентом Пралине, приобретайте качественные десерты по выгодным оптовым ценам!';

export const PAGE_DELIVERY_TITLE = 'Доставка';
export const META_DELIVERY_TITLE =
  'Доставка замороженных десертов бесплатно, бережно, качественно по Москве и ближайшему подмосковью.';
export const META_DELIVERY_DESCRIPTION =
  'Вся продукция доставляется в замороженном виде. Бесплатно, бережно, качественно и вовремя доставляем пирожные и торты по Москве и ближайшие пригородные районы.';

export const PAGE_NEWS_TITLE = 'Новости';
export const META_NEWS_TITLE = 'Новости кондитерской Пралине';
export const META_NEWS_DESCRIPTION =
  'Что новенького в нашем ассортименте. Акции и предложения';

export const PAGE_PRODUCTS_TITLE = 'Продукция';
export const PAGE_PRODUCTS_LINK = 'вся продукция';
export const META_PRODUCTS_TITLE =
  'Продукция компании Пралине - это качественные кондитерские изделия оптом для московских кафе, баров, пекарен, театров.';
export const META_PRODUCTS_DESCRIPTION =
  'Вкусные торты, пирожные, десерты. Широкий ассортимент по оптовым ценам с бесплатной доставкой. Идеальный выбор для предприятий общественного питания.';

export const PAGE_RECIPES_TITLE = 'Рецепты';
export const META_RECIPES_TITLE = 'Рецепты от кондитерской Пралине';
export const META_RECIPES_DESCRIPTION =
  'Сладкие рецепты десертов от нашего шеф повара.';

export const PAGE_CAKES_TITLE = 'Торты';
export const PAGE_CAKES_LINK = 'все торты';
export const META_CAKES_TITLE =
  'Замороженные торты от профессиональной кондитерской Пралине. Купить по оптовой цене.';
export const META_CAKES_DESCRIPTION =
  'Вкусные торты ручного приготовления, для пекарен, кофеен и магазинов. Медовый, птичье молоко, Прага, тирамису, три шоколада, наполеон, чизкейк и многие другие.';

export const PAGE_PASTRY_TITLE = 'Пирожные';
export const PAGE_PASTRY_LINK = 'все пирожные';
export const META_PASTRY_TITLE =
  'Замороженные пирожные по оптовым ценам, от профессиональной кондитерской Пралине для кофеен и театров.';
export const META_PASTRY_DESCRIPTION =
  'Выбирайте и покупайте вкусные десерты ручного приготовления от производителя. Эклеры, корзиночки, макаруны, штрудели и многое другое.';

export const PAGE_CATEGORIES_TITLE = 'Категории';
export const PAGE_CATEGORIES_LINK = 'все категории';
export const META_CATEGORIES_TITLE =
  'Категории пирожных которые готовятся в московской кондитерской Пралине.';
export const META_CATEGORIES_DESCRIPTION =
  'Выбирайте категории кондитерских изделий. Шоколадные, медовые, фруктовые, штрудель, бисквитные, сырные, кремовые, ореховые, маковые и другие.';

export const PAGE_PRICE_TITLE = 'Прайс-лист';
export const META_PRICE_TITLE =
  'Прайс-лист, каталог десертов кондитерской Пралине.';
export const META_PRICE_DESCRIPTION =
  'Полный перечень продукции компании Пралине.';

export const PAGE_STORAGE_TITLE = 'Хранение и разморозка';
export const META_STORAGE_TITLE =
  'Как правильно хранить замороженные десерты. Как размораживать пирожные и торты.';
export const META_STORAGE_DESCRIPTION =
  'Обязательный ряд правил по хранению замороженных десертов. Как правильно размораживать и как хранить в размороженном виде. Товарное соседство и маркировка товара.';

export const PAGE_STORIES_TITLE = 'Истории';
export const META_STORIES_TITLE =
  'Истории происхождения десертов, тортов, пирожных.';
export const META_STORIES_DESCRIPTION =
  'Истории о том, как появились десерты. Откуда взялись торты. Происхождение пирожных.';

export const PAGE_PTICHKA_TITLE = 'Птичье Молоко';
export const META_PTICHKA_TITLE =
  'История происхождения торта и конфет "Птичье Молоко"';
export const META_PTICHKA_DESCRIPTION =
  'Как появился торт и конфеты Птичье Молоко. Интересная история происхождения.';

export const PAGE_MURAVEINIK_TITLE = 'Муравейник';
export const META_MURAVEINIK_TITLE = 'Как появился торт "Муравейник"';
export const META_MURAVEINIK_DESCRIPTION =
  'История происхождения торта "Муравейник". Торт состоял из кусочков обычного печенья, смешанного с кремом из сгущенного молока.';

export const PAGE_NAPOLEON_TITLE = 'Наполеон';
export const META_NAPOLEON_TITLE =
  'Как Наполеон из императора стал русским тортом';
export const META_NAPOLEON_DESCRIPTION =
  'Этот торт, первоначально названный mille-feuille, был привезен в Россию в начале 19-го века и был широко приготовлен во время праздничных мероприятий после победы над армией Наполеона Бонапарта в 1812 году.';

export const PAGE_ESTERHAZY_TITLE = 'Эстерхази';
export const META_ESTERHAZY_TITLE = 'Знаменитый торт из Будапешта';
export const META_ESTERHAZY_DESCRIPTION =
  'Торт Эстерхази был создан в конце 19 века неизвестным кондитером Будапешта и вскоре стал одним из самых известных десертов на землях Австрии и Венгрии. Свое название торт получил в честь принца Павла Антона III Эстерхази де Галанта.';
